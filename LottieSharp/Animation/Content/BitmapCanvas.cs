...
namespace LottieSharp.Animation.Content
{
    public class BitmapCanvas : IDisposable
    {
        ...
        {
            public RenderTargetSave(DeviceContext renderTarget, int paintFlags, PorterDuffXfermode paintXfermode,
                byte paintAlpha)
            {
        ...
        public void DrawRect(double x1, double y1, double x2, double y2, Paint paint)
        {
            ...
            using (var brush = new SolidColorBrush(CurrentRenderTarget, paint.Color.ToRaw()))
            {
                ...
                {
                    CurrentRenderTarget.DrawRectangle(
                        new RawRectangleF((float) x1, (float) y1, (float) (x2 - x1), (float) (y2 - y1)), brush,
                        paint.StrokeWidth, GetStrokeStyle(paint));
                }
                else
                {
                    CurrentRenderTarget.FillRectangle(
                        new RawRectangleF((float) x1, (float) y1, (float) (x2 - x1), (float) (y2 - y1)), brush);
                }
            }
        ...
        private RawMatrix3x2 GetCurrentTransform()
        {
        ...
        public void SaveLayer(RectangleF bounds, Paint paint, int flags, Path path = null)
        {
            ...
            if (isClipToLayer)
            {
                ...
                _renderTargetSaves.Push(new RenderTargetSave(rendertarget.RenderTarget, paint.Flags, paint.Xfermode,
                    paint.Xfermode != null ? (byte) 255 : paint.Alpha));

                //var drawingSession = rendertarget.CreateDrawingSession();
                rendertarget.RenderTarget.Clear(new RawColor4(Color.Transparent.R, Color.Transparent.G,
        ...
        private RenderTargetHolder CreateRenderTarget(RectangleF bounds, int index)
        {
            if (!_renderTargets.TryGetValue(index, out var rendertarget))
            {
                ...
                var rt = new DeviceContext(CurrentRenderTarget.Device,
        ...
        public void DrawBitmap(Bitmap bitmap, RectangleF src, RectangleF dst, Paint paint)
        {
            ...
            CurrentRenderTarget.DrawBitmap(bitmap, dst.ToRaw(), paint.Alpha / 255f,
                BitmapInterpolationMode.NearestNeighbor, src.ToRaw());
        }
